...
WHERE E.SALARY >= 4000000 AND J.JOB_CODE = 'J2';;
...
AND HIRE_DATE < TO_DATE('20020101', 'YYYY/MM/DD');

--11. EMPLOYEE테이블에 고용일이 90/01/01 ~ 01/01/01인 사원의 전체 내용을 조회
SELECT *
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN TO_DATE('19900101', 'YYYY/MM/DD') AND TO_DATE('20010101', 'YYYY/MM/DD');

--12. EMPLOYEE테이블에서 이름 끝이 '연'으로 끝나는 사원의 이름 조회
SELECT EMP_NAME
FROM EMPLOYEE
--WHERE SUBSTR(EMP_NAME, -1, 1) = '연';
...
AND (HIRE_DATE BETWEEN TO_DATE('19900101', 'YYYY/MM/DD') AND TO_DATE('20001201', 'YYYY/MM/DD'))
AND (SALARY >= 2700000);

--15. EMPLOYEE테이블에서 사원 명과 직원의 주민번호를 이용하여 생년, 생월, 생일 조회
SELECT (1900 + SUBSTR(EMP_NO, 1, 2)) AS "생년", SUBSTR(EMP_NO, 3, 2) AS "생월", SUBSTR(EMP_NO, 5, 2) AS "생일"
FROM EMPLOYEE;
...
SELECT EMP_NAME, TO_CHAR(SALARY, 'L9999999')
FROM EMPLOYEE;
...
-- (단, 부서코드 오름차순으로 정렬)
SELECT DECODE(DEPT_CODE, 'D5', '총무부', 'D6', '기획부', 'D9', '영업부') AS "부서코드"
FROM EMPLOYEE;
