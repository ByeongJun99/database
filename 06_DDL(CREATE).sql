...
/*
    ...
    * 자료형
     - 문자(CHAR(바이트 크기) | VARCHAR2(바이트 크기))
        CHAR : 최대 2000바이트까지 지정 가능 / 고정길이(고정된 글자 수의 데이터가 담길 경우)
        (지정한 크기보다 더 작은 값이 들어오면 공백으로라도 채워서 처음 지정한 크기를 만들어준다.)
        VARCHAR2 : 최대 4000바이트까지 지정 자능 / 가변길이(몇 글자의 데이터가 담길 지 모르는 경우)
...
INSERT INTO MEMBER
VALUES(1, 'USER2', 'PASS2', '홍길순', NULL, NULL, NULL, SYSDATE);
...
/*
    * UNIQUE 제약조건
    ...
    삽입 / 수정 시 기존에 있는 데이터값 중 중복값이 있을 경우 오류를 발생한다.
*/
...
--> 만일 GENDER 컬럼에 데이터를 넣고자한다면 CHECK 제약조건에 만족하는 값을 넣어야한다.
...
-- 복합키 : 두 개의 컬럼을 동시에 하나의 PRIMARAY KEY로 지정하는 것
...
--------------------------------------------------------------------------------
/*
    자식 테이블 생성 시 외래키 제약조건 부여할 때 삭제 옵션 지정가능
    * 삭제옵션 : 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 할 것인가?
     ...
     - ON DELETE CASCADE : 부모데이터 삭제 시 해당 데이터를 사용하고 있는 자식데이터도 같이 삭제시키는 옵션
*/
DROP TABLE MEM;
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 10;
--> 잘 삭제가 완료됨, 10을 가져다 쓰고 있던 자식데이터의 값은 NULL로 변경됨
...
--> 삭제가 잘됨, 해당 데이터를 사용하고 있던 자식 데이터도 같이 삭제됨.
...
/*
    * 테이블이 다 생성된 후에 뒤늦게 제약조건을 추가하는 방법
    ALTER TABLE 테이블명 변경할 내용
     ...
     - FOREIGN KEY : ALTER TABLE 테이블명 ADD FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블명;
