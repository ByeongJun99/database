...
/*
   ...
   FROM 조회할 테이블
   WHERE 조건식
   ORDER BY 정렬기준이 될 컬럼명 | 별칭 | 컬럼순번 [ASC | DESC] [NULLS FIRST | NULL LAST]
...
--ORDER BY BONUS ASC; -- NULLS LAST 기본
...
SELECT EMP_NAME, HIRE_DATE, LAST_DAY(HIRE_DATE), (LAST_DAY(HIRE_DATE) - HIRE_DATE) AS "입사달의 근무일수"
FROM EMPLOYEE;
...
SELECT TO_CHAR(1234, '99999') AS "NUMBER" FROM DUAL; -- 공간확보, 오른쪽 정렬, 빈칸공백
...
SELECT TO_CHAR(SYSDATE, 'YYYY'),
        ...
        TO_CHAR(SYSDATE, 'RR') -- RR룰이 따로 존재 -> 50년 이상 값 : + 100 -> 1954
FROM DUAL;

SELECT HIRE_DATE, TO_CHAR(HIRE_DATE, 'YY'), TO_CHAR(HIRE_DATE, 'RR') FROM EMPLOYEE;

-- 월에 관련된 포맷
SELECT TO_CHAR(SYSDATE, 'MM'), -- 이번달 숫자 두 자리
        TO_CHAR(SYSDATE, 'MON'),
...
-- 4. MAX(모든타입가능) : 해당 컬럼값 중 가장 큰 값 구해서 반환
