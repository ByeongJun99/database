/*
    ...
    (무작정 다 조회해 오는 것이 아니라 각 테이블 간 연결고리(외래키)를 통해 데이터를 매칭시켜 조회해야한다.)
...
-- NULL, D3, D4, D7데이터는 한쪽 테이블에서만 존재하기 때문에 제외된 걸 알 수 있다.
...
-- JOIN USING같은 경우 연결하는 컬럼명이 같을 때만 사용 가능
...
/*
    ...
    단, 반드시 LEFT/RIGHT를 지정해야된다. (기준 테이블을 정해야한다.)
*/
-- 사원명, 부서명, 급여, 연봉
-- 내부JOIN시 부서 배치를 받지 않은 2명의 사원 정보가 누락된다.
SELECT *
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
...
/*
    ...
    매칭시킬 컬럼에 대한 조건 작성 시 '='을 사용하지 않는 JOIN문
    ANSI구문으로는 JOIN ON
*/
