/*
    ...
    EX)
    회원 탈퇴시 기존의 회원 테이블에 데이터 DELETE 후 곧바로 탈퇴한 회원들만 따로 보관하는 테이블에
    자동으로 INSERT 시켜야한다.
    신고 횟수가 일정 수를 넘었을 때 묵시적으로 해당 회원을 블랙리스트로 처리되게끔
    입출고에 대한 데이터가 기록(INSERT)될 때마다 해당 상품에 대한 재고 수량을 매번 수정(UPDATE)해야한다.
    ...
    * 트리거 생성 구문
    ...
    [DECLARE 변수선언]
    BEGIN
        실행내용(해당 위에 지정된 이벤트 발생 시 묵시적(자동)으로 실행할 구문)
    [EXCEPTION 예외처리]
    END;
    /
*/
...
-- 샘플데이터 추가
INSERT INTO TB_PRODUCT VALUES(SEQ_PCODE.NEXTVAL, '갤럭시S24', '샘송', 1400000, DEFAULT);
...
-- TB_PRODUCT 테이블에 매번 자동으로 재고수량 UPDATE되게끔 트리거 작성
